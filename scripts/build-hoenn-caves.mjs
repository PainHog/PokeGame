/**
 * Pokémon Masters — authentic Gen-3 (Emerald / Hoenn) CAVE / DUNGEON / SPECIAL map
 * renderer + metadata. Sibling of build-emerald-maps.mjs; reuses the exact same
 * compositing pipeline (RSE split, 16×16 metatile draw, ×SCALE upscale, WebP encode,
 * warp/connection/grass extraction) — only the map list and the output filenames
 * differ. Covers the remaining Hoenn interior/exterior locations that build-emerald-
 * maps.mjs (towns + routes) does not.
 *
 *   node scripts/build-hoenn-caves.mjs                 (render all cave/dungeon maps)
 *   node scripts/build-hoenn-caves.mjs "Granite Cave"  (a single map)
 *
 * Writes assets/maps/gba-hoenn-caves.json (SAME schema as gba-hoenn.json) plus one
 * assets/maps/<slug>.webp per rendered map.
 *
 ...
 * Source: github.com/pret/pokeemerald (open decomp; tile art © Nintendo/GF).
 */
...
const MB_LONG_GRASS = 0x03;      // waist-high grass — also a wild-encounter zone

// Our scene name → [Emerald blockdata dir (CamelCase), kind, explicit slug, skipReason?].
// The slug is spelled out (not derived) so the json key matches our scene name exactly.
// A null dir means the location has NO clean standalone decomp map worth rendering —
// it is intentionally SKIPPED (do not invent art) and the reason is reported.
const CAVES = {
  "Granite Cave":         ["GraniteCave_1F",                "cave",  "granite-cave"],
  "Meteor Falls":         ["MeteorFalls_1F_1R",             "cave",  "meteor-falls"],
  "Mt. Chimney":          ["MtChimney",                     "route", "mt-chimney"],
  "Mt. Pyre":             ["MtPyre_Exterior",               "route", "mt-pyre"],
  "Cave of Origin":       ["CaveOfOrigin_Entrance",         "cave",  "cave-of-origin"],
  "Seafloor Cavern":      ["SeafloorCavern_Entrance",       "cave",  "seafloor-cavern"],
  "Shoal Cave":           ["ShoalCave_LowTideEntranceRoom", "cave",  "shoal-cave"],
  "Sky Pillar":           ["SkyPillar_Outside",             "route", "sky-pillar"],
  "Fiery Path":           ["FieryPath",                     "cave",  "fiery-path"],
  "Jagged Pass":          ["JaggedPass",                    "route", "jagged-pass"],
  "Rusturf Tunnel":       ["RusturfTunnel",                 "cave",  "rusturf-tunnel"],
  "Hoenn Victory Road":   ["VictoryRoad_1F",                "cave",  "hoenn-victory-road"],
  // Route121_SafariZoneEntrance warps INTO SafariZone_South → that is the main entrance area.
  "Hoenn Safari Zone":    ["SafariZone_South",              "route", "hoenn-safari-zone"],
  // OutsideEast holds the Battle Tower + the Battle Frontier Pokémon Center → kind "town".
  "Battle Frontier":      ["BattleFrontier_OutsideEast",    "town",  "battle-frontier"],
  // --- intentionally skipped (no clean standalone exterior map in the decomp) ---
  "Weather Institute":    [null, "route", "weather-institute",
    "no standalone exterior map — the Weather Institute exterior is baked into Route119; only interior floors (Route119_WeatherInstitute_1F/2F, Building tileset) exist standalone"],
  "Hoenn Pokémon League": [null, "cave", "hoenn-pokémon-league",
    "no clean standalone league exterior — the exterior is the north half of EverGrandeCity (already rendered as its own scene); only interior EverGrandeCity_PokemonLeague_1F (Building tileset) exists standalone"],
};
...
async function main() {
  ...
  const entries = Object.entries(CAVES).filter(([n]) => !only.length || only.includes(n));
  const layouts = (await fetchJson(`${EM}/data/layouts/layouts.json`))?.layouts.filter(Boolean) ?? [];
  const outPath = path.join(OUT, "gba-hoenn-caves.json");
  ...
  console.log(`${metaOnly ? "Injecting collision into" : "Rendering"} ${entries.length} authentic Gen-3 Hoenn cave/dungeon/special maps${metaOnly ? "" : ` (×${SCALE})`}…`);
  let meta = {};
  ...
  for (const [ourName, [dir, kind, slug, skipReason]] of entries) {
    if (!dir) { skipped.push([ourName, skipReason || "no decomp match"]); console.warn(`  ⤫ SKIP ${ourName}: ${skipReason || "no decomp match"}`); continue; }
    try {
      const layout = layouts.find((l) => (l.blockdata_filepath || "").split("/")[2]?.toLowerCase() === dir.toLowerCase());
      if (!layout) { skipped.push([ourName, `no layout dir matched ${dir}`]); console.warn(`  ! no layout for ${dir}`); continue; }
      ...
      if (metaOnly) {
        if (meta[slug]) meta[slug].collision = collision;
        else meta[slug] = { name: ourName, kind, w, h, grid: GRID, warps: [], connections: [], grass: null, collision };
        console.log(`  · ${ourName.padEnd(20)} ${slug.padEnd(20)} collision ${w}×${h}`);
        ok++;
        continue;
      }
      ...
      // Warps + connections from the map header (layout dir == map dir).
      ...
      meta[slug] = { name: ourName, kind, w, h, grid: GRID, warps, connections: conns, grass: gb, collision };
      ...
      await fs.writeFile(path.join(OUT, `${slug}.webp`), Buffer.from(url.split(",")[1], "base64"));
      console.log(`  ✓ ${ourName.padEnd(20)} ${slug.padEnd(20)} ${OW * SCALE}×${OH * SCALE}  warps:${warps.length} conns:${conns.length} grass:${grass.length}`);
      ok++;
    } catch (e) { skipped.push([ourName, e.message]); console.warn(`  ! ${ourName}: ${e.message}`); }
  }
  if (browser) await browser.close();
  await fs.writeFile(outPath, JSON.stringify(meta, null, 1));
  console.log(`Done — ${ok}/${entries.length} ${metaOnly ? "collision grids injected" : "rendered"}. Wrote assets/maps/gba-hoenn-caves.json.`);
  if (skipped.length) { console.log("Skipped:"); for (const [n, r] of skipped) console.log(`  - ${n}: ${r}`); }
}
