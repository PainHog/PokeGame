...
function findChrome() { const base = "/opt/pw-browsers"; try { for (const d of readdirSync(base)) { if (d.startsWith("chromium-")) { const p = path.join(base, d, "chrome-linux", "chrome"); if (existsSync(p)) return p; } } } catch { /* fall through */ } return null; }
...
async function main() {
  ...
  console.log(`Rendering ${entries.length} authentic Gen-3 Hoenn cave/dungeon/special maps (×${SCALE})…`);
  ...
  for (const [ourName, [dir, kind, slug, skipReason]] of entries) {
    ...
    try {
      ...
      meta[slug] = { name: ourName, kind, w, h, grid: GRID, warps, connections: conns, grass: gb };
  ...
  await browser.close();
  await fs.writeFile(path.join(OUT, "gba-hoenn-caves.json"), JSON.stringify(meta, null, 1));
  console.log(`Done — ${ok}/${entries.length} rendered. Wrote assets/maps/gba-hoenn-caves.json.`);
