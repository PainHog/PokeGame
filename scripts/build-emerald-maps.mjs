...
async function main() {
  ...
  console.log(`Rendering ${entries.length} authentic Gen-3 Hoenn maps (×${SCALE})…`);
  ...
  await browser.close();
  await fs.writeFile(path.join(OUT, "gba-hoenn.json"), JSON.stringify(meta, null, 1));
  console.log(`Done — ${ok}/${entries.length} rendered. Wrote assets/maps/gba-hoenn.json.`);
}
